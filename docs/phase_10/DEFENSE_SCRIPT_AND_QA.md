...
- Causal panel provides observational treatment-effect estimates (matching, ATE, confidence interval, and counterfactual scenarios) with explicit assumptions.
...
- Results are district-level and observational; causal panel outputs are assumption-dependent and not randomized proof.
...
| Are these causal claims about migration/background? | Ethical and methodological risk | Clarify the causal panel is observational (matching-based), assumption-bound (no unmeasured confounding, positivity), and intended for policy prioritization rather than proof | Causal methodology note in dashboard + phase docs |
